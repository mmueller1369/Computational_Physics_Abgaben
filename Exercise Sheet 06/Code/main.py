...
B2s = [tools_6.B2_red(beta, eps) for eps in epss]
eps_argon = settings.eps * beta
print(f"For Argon: eps^* = {eps_argon}, B_2^* = {tools_6.B2_red(beta, settings.eps)}")

plt.figure(figsize=(6, 4))
plt.plot(epss * beta, B2s, label=r"$B_2^*$", color="b")
plt.axvline(eps_argon, label=r"$\epsilon^*_\mathrm{Argon}$", color="r")
