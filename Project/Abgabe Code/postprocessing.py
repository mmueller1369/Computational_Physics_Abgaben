...
def AnalyseMoleculesModifier(frame, data):
    ...
    for mol, molID in enumerate(molIDs):
        ...
        dipoles[mol] = -(si + sj) / (2 * settings.s0 * math.cos(settings.theta0 / 2))
...
def MeasurePartCOMDistanceModifier(frame, data):
...
def fermi_distribution(r, r0, rho_bulk, a):
    exponent = (r-r0)/a
    return rho_bulk / (np.exp(exponent)+1)
...
class PostprocessingTools:
    def __init__(self, filename, every_nth_frame=1, cutoff=settings.cutoff):
        ...
        self.pipeline.modifiers.append(MeasurePartCOMDistanceModifier)
    ...
    def calculate_rho(self, step=None, rmax_hist=settings.rmax_hist, dr_hist=settings.dr_hist, only_hatoms=False):
        ...
        # params, pcov = curve_fit(fermi_distribution, dist[3:], rho[3:], p0=[1.15, 34, 0.1])
    ...
    def calculate_electrostatic_potential_and_field(self, step=None, rmax_hist=settings.rmax_hist, dr_hist=settings.dr_hist, fitted_densities=False):
        ...
        for i, r in enumerate(dist):
            ...
            factor = 0.0433641039  # conversion factor from kcal/mole / e to J/C = V
            potential[i] = 1/settings.eps0_el * 1/r * integral * factor
        field = -np.gradient(potential, dist) # in V/nm
