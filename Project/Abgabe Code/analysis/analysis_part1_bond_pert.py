...
plt.axhline(np.mean(si[:1500]), color='r', linestyle='--', label=r"$\langle s_i \rangle$ = " + f"{np.mean(si[:1500]):.2f} nm")
plt.plot(steps[:1500], sj[:1500], label=r"$s_j$")
plt.axhline(np.mean(sj[:1500]), color='g', linestyle='--', label= r"$\langle s_j \rangle$ = " + f"{np.mean(sj[:1500]):.2f} nm")
plt.xlabel(r"$t$ [fs]")
plt.ylabel(r"$s_i$ [nm]")
plt.legend()
...
try:
    si_arr = si[100:800] # select data here
    ...
    if len(peaks) > 1:
        ...
        print(f"Variance of slope: {np.sqrt(cov[0,0]):.2E}")
