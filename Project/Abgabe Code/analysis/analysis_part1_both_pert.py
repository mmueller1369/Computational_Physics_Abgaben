"""
Configuration C
First you get the energy plot.
Secondly you can get the bond length evoultion and theta evolution.
Thirdly you can get the maxima and linear regression plot. 

Watch out! Many hard coded variables. Make sure to use right variables.
"""
...
# First 1500 steps 
t_short0 = energies[0][:1500] * 0.5  # Convert to fs
axs[0].plot(t_short0, energies[3][:1500], label = r"$E_{bond}$")
axs[0].plot(t_short0, energies[4][:1500], label = r"$E_{angle}$")
axs[0].plot(t_short0, energies[5][:1500], label = r"$E_{kin}$")
axs[0].plot(t_short0, np.sum(energies[1:, :1500], axis=0), label = r"$E_{tot}$")
#axs[0].set_title(f'Zoom into first 1500 steps')
...
#axs[1].set_title(f' Zoom into first 100 steps)')
...
#theta evolution
plt.figure(figsize=(10, 5))
plt.plot(steps[:1500], theta[:1500] * 180 / np.pi, label=r"$\Theta$")
...
try:
    ...
    if len(peaks) > 1:
        ...
        error = np.sqrt(cov[0,0] )*1e-15
        print(f"Error in slope: {error:.2E}s per maxima")
