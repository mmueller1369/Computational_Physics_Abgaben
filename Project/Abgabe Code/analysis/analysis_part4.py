...
plt.axhline(bulk, label=r'$\hat\rho_\text{O}$', color='purple')
# plt.fill_between(rfit,
#                  bulk+ubulk, bulk-ubulk, color='purple', alpha=.2)
plt.axvline(radius, label=r'$R_\text{O}$', color='blue')
# plt.fill_between([radius+uradius, radius-uradius],
#                  0, 40, color='blue', alpha=.2)
plt.axvline(radius+sharp, label=r'$R_\text{O} \pm a_\text{O}$', color='orange')
plt.axvline(radius-sharp, color='orange')
plt.plot(rfit, fermi_distribution(rfit, *params), label=r'$\langle\rho_\text{O}(r)\rangle_t$ fit', color='black')
plt.legend()
plt.xlabel(r'$r$ [nm]')
...
print(f"Minimum potential: {np.min(pot)} V")
fig, ax1 = plt.subplots()
color1 = 'blue'
ax1.set_xlabel(r'$r$ [nm]')
ax1.axhline(0, color=color1, ls='dashed')
ax1.set_ylabel(r'$\Phi$ [V]', color=color1)
ln1 = ax1.plot(dist, pot, color=color1, label=r'$\Phi$')
...
ax2 = ax1.twinx()
color2 = 'red'
ax2.set_ylabel(r'$E$ [V/nm]', color=color2)
ax2.axhline(0, color=color2, ls='dashed')
ln3 = ax2.plot(dist, field, color=color2, label=r'$E$')
# ln4 = ax1.plot(rfit, fieldfit, color=color2, label=r'$E fit$', ls='--')
ax2.tick_params(axis='y', labelcolor=color2)
...
ax1.set_xlabel(r'$t$ [ps]')
ax1.set_ylabel(r'$T$ [K]', color=color1)
ln1 = ax1.plot(time, temperature-temperature_all, label=r"$\Delta T$", color='black')
ax1.tick_params(axis='y', labelcolor=color1)
