...
k_angle = 75 * 4184 *1e3 #directly converting units
...
plt.axhline(np.mean(theta[:1500]) * 180 / np.pi, color='r', linestyle='--', label=r"$\langle \Theta \rangle$ = " + f"{np.mean(theta[:1500]) * 180 / np.pi:.2f}°")
plt.xlabel(r"$t$ [fs]")
plt.ylabel(r"$\Theta$ [°]")
plt.legend()    
...
# doesnt work as good as expected
